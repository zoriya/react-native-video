...
const writeToPodfile = (projectRoot, key, value, testApp = false) => {
  const podfilePath = _nodePath.default.join(projectRoot, 'ios', 'Podfile');
  const podfileContent = _nodeFs.default.readFileSync(podfilePath, 'utf8');
...
const mergeTestAppPodfile = (podfileContent, podfilePath, key, value) => {
  ...
  if (newPodfileContent.didMerge) {
    _nodeFs.default.writeFileSync(podfilePath, newPodfileContent.contents);
  } else {
