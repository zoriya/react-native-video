...
const useManagedInstance = (config, dependencies) => {
  ...
  const [released, setReleased] = (0, _react.useState)(false);
  if (objectRef.current == null) {
    objectRef.current = factory();
  }
  const object = (0, _react.useMemo)(() => {
    let newObject = objectRef.current;
    const dependenciesChanged = previousDependencies.current?.length === dependencies.length && dependencies.every((value, index) => dependenciesEqualFn?.(value, previousDependencies.current[index]) ?? value === previousDependencies.current[index]);
