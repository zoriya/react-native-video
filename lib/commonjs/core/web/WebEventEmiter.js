...
class WebEventEmiter {
  ...
  addOnAudioTrackChangeListener(listener) {
    return this._addListener("onAudioTrackChange", listener);
  }
  addOnVideoTrackChangeListener(listener) {
    return this._addListener("onVideoTrackChange", listener);
  }
  addOnQualityChangeListener(listener) {
  ...
  _onError() {
    ...
    this._emit("onError", new _VideoError.VideoError(codeMap[err.code], err.message));
  }
  _onTextTrackChange() {
