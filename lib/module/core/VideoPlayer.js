...
class VideoPlayer extends VideoPlayerEvents {
  ...
  throwError(error) {
    ...
    if (parsedError instanceof VideoRuntimeError && this.triggerJSEvent('onError', parsedError)) {
  ...
  async replaceSourceAsync(source) {
    this.updateMemorySize();
    await this.wrapPromise(this.player.replaceSourceAsync(source === null ? null : createSource(source)));
    this.updateMemorySize();
  }
